...
int icopy(int fd)   //将可能需要用到的i节点结构拷贝到内存中
{
	...
	for (i = 0; i < 32; i++)
		...
		if (i == 32)
		{
			cout << "There are too many files have been open!\n"
			     << "Please input the file number you want to close:";
			cin >> fd1;
			i = fclose(fd1);
			return -1;
		}
...
int fcreate(char *name, int mode)   //在根目录下创建名为name的文件
{
	...
	{
		cout << "There is no space to create a file!" << endl;
		return -1;
	}
	if (mode != 1 && mode != 0)
	{
		cout << "Please input the right type of file!" << endl;
		return -2;
	}
	if (g_spb.s_ninode >= 0)
	{
	...
	{
		cout << "The number of file is max! Please delete some files!" << endl;
		return -3;
	}
	//int a = 0;
...
int fopen(char *name, int mode) //读文件名为str的文件入内存i节点，成功则返回内存i节点编号
{
	...
	if (inode[j].i_mode == Inode::IFDIR)
	{
		...
		else
		{
			...
			cout << "Open success, the node number is: " << j << endl;
			return j;
		}
	}
	return 1;
...
int copyin(char *srcpath, char *name)    //将虚拟磁盘外的文件拷贝入虚拟磁盘
{
	...
	fread(m_Buf[0].b_addr, 1, filesize % 512, fp1);//多余512字节的剩余字节拷贝
	flseek(fd, i * 512);
	fwrite(fd, (char*)Buffer[0], filesize % 512);
	flseek(fd, 0);
	m_Buf[0].b_blkno = -1;
	m_Buf[0].b_flags = 0;
